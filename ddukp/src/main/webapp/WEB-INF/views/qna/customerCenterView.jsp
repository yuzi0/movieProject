...
<script type="text/javascript">
	$(document).ready(function(){
		$("#btn-left1").on('click',function(){
			...
			if(chk){
				location.href='customerCenterDelete.do?qnum=${param.qnum}';
			}
		});

		$("#list").on('click',function(){
			var board = '${param.board}';
			//alert('param : ${param.board}');
			if(board == "qna") {
						...
						success : function(data) {
							...
							if (data.length > 0) {
								for (var i = 0; i < data.length-1; i++) {
									...
									chtml+="<div class='"+data[i].qcnum+"' id='com'>";
									chtml+="<h5 class='name'>"+data[i].number+"</h5>";
									chtml+="<p>"+data[i].comment+"</p>";
									chtml+=	"<span class='comment-date'>"+data[i].qcdate+"</span>&nbsp;&nbsp;&nbsp;"; 
							if(data[data.length-1].anum>0||data[i].uid===('<%=session.getAttribute("sid")%>')){
	...
	$('.comments-list').on('click','#deletebtn',function(){
		$.ajax({
			type : 'get',
			url : 'customerCommentDelete.do',
			data :{ qcnum:$('#com').attr('class')},
				...
				<ul class="nav navbar-nav">
					...
					<li><a class="active" href="infoBoard.do"> Q & A <i
						...
						<ul class="dropdown">
							...
							<li><a class="active" href="customerCenter.do"> 고객센터 </a></li>
						</ul></li>
			...
			<div class="row">
				<div class="col-md-12">
					<div class="breadcrumb-wrapper">
						<h2 class="product-title">고객센터 글 상세페이지</h2>
						<ol class="breadcrumb">
							<li><a href="#"><i class="ti-home"></i> MOVIEP</a></li>
							<li class="current" style="color: #17a2b8;">고객센터 글 상세페이지</li>
						</ol>
					</div>
				</div>
			</div>
			...
			<div class="col-md-12 col-sm-12 col-xs-12">
				<div class="">
					<table class="table table-borderless table-sm">
						...
						<tr>
							<th class="col-md-5" style="text-align: center;">[${qTO.qctgname }]&nbsp&nbsp&nbsp <b>${qTO.qsubject }</b></th>
							<th class="col-md-3" style="text-align: right;">${qTO.qdate }</th>
							<th class="col-md-2" style="text-align: right;">${qTO.qhit }</th>
							<th class="col-md-2" style="text-align: right;">${qTO.commentNum }</th>
						</tr>
					</table>

					<div>${qTO.qcontent }</div>

				</div>
				<div class="col-md-12" style="padding-top: 30px">
					<a id="list" href="customerCenter.do?cpage=${param.cpage}" class="btn btn-common pull-left">목록</a> 
					<c:if test="${!empty sid && sid eq qTO.uid }">
					
					<a href='customerCenterModify.do?qnum=${param.qnum}&cpage=${param.cpage}' class="btn btn-common pull-right">수정</a> 
					</c:if>
						
						 <c:if test="${(!empty sid && sid eq qTO.uid) || qTO.anum > 0}">
		...
		<div class="container">
			<div id="comments" class="col-md-10 col-md-offset-1">
				...
				<div id="respond">
					...
					<input type="hidden" name="qnum" value="${qTO.qnum}"> 
